...
import           Finite                         ( Finite(index, offset)
                                                , FiniteBounds
                                                , v2t
                                                )
...
indent :: Int -> String
indent n = let indentLevel = "  " in "\n" ++ concat (replicate n indentLevel)

strInd :: (Finite HOA a, FiniteBounds HOA) => a -> String
strInd a = show (index a - offset (v2t a))

strIndWithMap
  :: (Finite HOA a, FiniteBounds HOA) => M.Map Int String -> a -> String
strIndWithMap nameMap a =
  translateToTSL $ nameMap M.! (index a - offset (v2t a))

translateToTSL :: String -> String
translateToTSL t = if "p0" `isPrefixOf` t
...
generateTSLString tslType decoder x =
  either show (T.tslFormula id . tslType) $ decoder x

-- | replace eg real3.4() with 3.4 and
--   int4() with 4
replaceTSLMT :: Text.Text -> Text.Text
replaceTSLMT x =
  let r typeName w = if typeName `Text.isPrefixOf` w
...
splitPredUpdates negationOperator terms =
  let sortTerm t (preds, upds) = if "<-" `isInfixOf` t
...
printTSLFormula negationOperator showVar = \case
  ...
  where printSubFormula = brRound . printTSLFormula negationOperator showVar
